...
if ( ! function_exists( 'qode_wishlist_for_woocommerce_plugin_installation' ) ) {
	function qode_wishlist_for_woocommerce_plugin_installation() {

		if ( isset( $_POST ) && isset( $_POST['plugin'] ) ) {
			$plugin_key = sanitize_text_field( wp_unslash( $_POST['plugin'] ) );
			check_ajax_referer( 'qode-wishlist-for-woocommerce-install-' . $plugin_key, 'nonce' );

			$plugin = qode_wishlist_for_woocommerce_get_plugin_by_slug_from_others_plugins( $plugin_key );
			...
			if ( 'install' === $plugin_action ) {
				...
				if ( ! is_wp_error( $install_result ) && $install_result ) {
					...
					if ( null === $activate ) {
						$button = qode_wishlist_for_woocommerce_plugin_get_plugin_link( $plugin_key, $plugin );
						qode_wishlist_for_woocommerce_get_ajax_status( 'success', esc_html__( 'Installed and activated', 'qode-wishlist-for-woocommerce' ), array( 'button' => $button ) );
					}
				}
			} else {
				$activate = activate_plugin( $plugin_slug, '', false, true );
				...
				if ( null === $activate ) {
					...
					$button = qode_wishlist_for_woocommerce_plugin_get_plugin_link( $plugin_key, $plugin );
					qode_wishlist_for_woocommerce_get_ajax_status( 'success', esc_html__( 'Activated', 'qode-wishlist-for-woocommerce' ), array( 'button' => $button ) );
				}
			}
	...
	add_action( 'wp_ajax_qode_wishlist_for_woocommerce_plugin_installation', 'qode_wishlist_for_woocommerce_plugin_installation' );
}
